...
function digestEmailHtml({ party, responses }) {
  ...
  const guests  = yes.reduce((s, r) => s + (r.guest_count || 0), 0);
  ...
  const rows = responses.map(r => {
    ...
    return `<tr>
      ...
      <td style="padding:9px 12px;font-size:0.78rem;color:#6b5c45;border-bottom:1px solid #f5edda;">${r.guest_count||'—'}</td>
      <td style="padding:9px 12px;font-size:0.78rem;color:#6b5c45;border-bottom:1px solid #f5edda;">${r.allergies||'—'}</td>
    </tr>`;
  }).join('');

  return base(`
    <p style="font-size:0.65rem;letter-spacing:0.2em;text-transform:uppercase;color:#c9a84c;margin:0 0 14px;">Daily RSVP summary 📋</p>
    <h1 style="font-family:Georgia,serif;font-size:1.9rem;font-weight:400;color:#2a2218;margin:0 0 14px;line-height:1.25;">${total} new response${total>1?'s':''} today<br>for <em style="color:#c9a84c;">${party.child_name}'s party</em></h1>
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;">
      <tr>
        ...
        <td width="33%" style="padding:0 3px;"><div style="background:#f5f0e8;border-radius:10px;padding:14px;text-align:center;"><div style="font-family:Georgia,serif;font-size:1.8rem;color:#a0907e;">${no.length}</div><div style="font-size:0.62rem;letter-spacing:0.1em;text-transform:uppercase;color:#a0907e;">Declined</div></div></td>
        ${guests>0?`<td width="33%" style="padding-left:6px;"><div style="background:#f5edda;border-radius:10px;padding:14px;text-align:center;"><div style="font-family:Georgia,serif;font-size:1.8rem;color:#c9a84c;">${guests}</div><div style="font-size:0.62rem;letter-spacing:0.1em;text-transform:uppercase;color:#c9a84c;">Guests</div></div></td>`:''}
      </tr>
    </table>
    <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #f5edda;border-radius:10px;overflow:hidden;margin-bottom:24px;">
      <thead><tr style="background:#f5edda;">
        ...
        <th style="padding:9px 12px;font-size:0.62rem;letter-spacing:0.1em;text-transform:uppercase;color:#a89880;text-align:left;font-weight:500;">Guests</th>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    sendEmail({
      ...
      html:    welcomeEmailHtml({ child_name, age, venue, dashboard_token, party_id }),
