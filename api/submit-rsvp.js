...
from: ‘Tiny Invites [hello@tinyinvites.org](mailto:hello@tinyinvites.org)’,
to,
subject,
html,
});
...
const heroBlock = photoUrl ? ` <tr><td style="padding:0;overflow:hidden;"> <img src="${photoUrl}" alt="Party" style="width:100%;max-height:200px;object-fit:cover;display:block;border-radius:12px 12px 0 0;"> </td></tr>` : ‘’;
...
if (!attending) {
return base(`<p style="font-size:0.62rem;letter-spacing:0.18em;text-transform:uppercase;color:#c9a84c;margin:0 0 12px;">We'll miss you 🥺</p> <h1 style="font-family:Georgia,serif;font-size:1.8rem;font-weight:400;color:#2a2218;margin:0 0 14px;line-height:1.3;">Thanks for letting us know, ${response.guest_name}</h1> <p style="font-size:0.87rem;color:#6b5c45;line-height:1.7;margin:0;"> We've let the host know you can't make it to <strong>${party.child_name}'s ${ageStr}</strong>. We hope to celebrate together another time. 💛 </p>`, party.photo_url || ‘’);
}
return base(`<p style="font-size:0.62rem;letter-spacing:0.18em;text-transform:uppercase;color:#c9a84c;margin:0 0 12px;">You're on the list! 🎉</p> <h1 style="font-family:Georgia,serif;font-size:1.8rem;font-weight:400;color:#2a2218;margin:0 0 14px;line-height:1.3;">See you there, ${response.guest_name}!</h1> <p style="font-size:0.87rem;color:#6b5c45;line-height:1.7;margin:0 0 16px;"> Your RSVP for <strong>${party.child_name}'s ${ageStr}</strong> is confirmed.${party.venue ?` We’ll see you at <strong>${party.venue}</strong>.`: ''} 🎈 </p> ${response.guest_count > 1 ?`<p style="font-size:0.82rem;color:#6b5c45;margin:0 0 10px;">👥 <strong>${response.guest_count} guests</strong> confirmed under your name</p>`: ''} ${response.allergies ?`<p style="font-size:0.82rem;color:#6b5c45;background:#fff3e0;border-left:3px solid #c9a84c;padding:10px 14px;border-radius:0 6px 6px 0;margin:0;">⚠️ Dietary note recorded: <strong>${response.allergies}</strong></p>`: ''}`, party.photo_url || ‘’);
}
...
return base(`<p style="font-size:0.62rem;letter-spacing:0.18em;text-transform:uppercase;color:#c9a84c;margin:0 0 12px;">New RSVP ${emoji}</p> <h1 style="font-family:Georgia,serif;font-size:1.8rem;font-weight:400;color:#2a2218;margin:0 0 16px;line-height:1.3;">${response.guest_name} ${status}</h1> ${attending && response.guest_count ?`<p style="font-size:0.85rem;color:#6b5c45;margin:0 0 8px;">👥 <strong>${response.guest_count} guest${response.guest_count > 1 ? ‘s’ : ‘’}</strong> attending</p>`: ''} ${response.allergies ?`<p style="font-size:0.85rem;color:#6b5c45;background:#fff3e0;border-left:3px solid #c9a84c;padding:10px 14px;border-radius:0 6px 6px 0;margin:0 0 12px;">⚠️ Dietary note: <strong>${response.allergies}</strong></p>`: ''} <p style="font-size:0.78rem;color:#a89880;margin:12px 0 0;">Any further responses today will be bundled into a daily summary.</p> ${btn(dashUrl, '📊 See full guest list →')}`, party.photo_url || ‘’);
}
