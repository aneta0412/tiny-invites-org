...
//        optional REMINDER_NOTE env var shown as a host note in the email.
...
// Reminder note is now stored per-party in parties.reminder_note (TEXT, nullable).
// The env var fallback is kept for emergencies but the DB value takes precedence.
...
// ── Note block (shared between reminder + update emails) ──
...
export default async function handler(req, res) {
  ...
  try {
      ...
      for (const guest of guests) {
        try {
          await withRetry(() => resend.emails.send({
            ...
            html:    buildReminderHtml({ party, guest, note: (party.reminder_note || '').trim() }),
            headers: {
    ...
    try {
      await resend.emails.send({
        ...
        html: `<p style="font-family:Arial,sans-serif;font-size:14px;color:#2a2218;line-height:1.8;">
          ...
          📝 <strong>Host note included:</strong> ${party.reminder_note ? 'Yes' : 'No'}<br/>
    ...
    return res.status(200).json({ sent, skipped, noteIncluded: parties.some(p => !!p.reminder_note) });
