...
function welcomeEmailHtml({ child_name, age, venue, party_date, dashboard_token, party_id, photo_url, rsvp_url }) {
  ...
  <tr><td style="background:#fff9f2;border:1px solid #f5edda;border-radius:14px;overflow:hidden;">
    <table width="100%" cellpadding="0" cellspacing="0">
      ...
      <tr><td style="padding:32px 40px 36px;">
        ...
        <p style="font-size:0.87rem;color:#6b5c45;line-height:1.7;margin:0 0 14px;">
          Your RSVP page is live. Share the link or QR code below with your guests — you'll get instant emails for first 15 responses each day. Any extra replies will arrive in your evening summary.
        </p>
        ${dateLine}
        ${venue ? `<p style="font-size:0.85rem;color:#6b5c45;margin:0 0 18px;">📍 <strong style="color:#2a2218;">${venue}</strong></p>` : ''}
      ...
      try {
        await resend.emails.send({
          ...
          html: `<p style="font-family:Arial,sans-serif;font-size:14px;color:#2a2218;line-height:1.8;">
            ...
            👶 <strong>Child:</strong> ${party.child_name}${party.age ? `, turning ${ordinal(party.age)}` : ''}<br/>
            📅 <strong>Date:</strong> ${formatPartyDate(party.party_date)}<br/>
            📍 <strong>Venue:</strong> ${party.venue || '—'}<br/>
