...
function contactHtml({ party, guestName, guestEmail, message }) {
  ...
  const replyRow = guestEmail
    ? `<p style="font-size:0.82rem;color:#6b5c45;margin:0 0 16px;">Reply directly to: <a href="mailto:${guestEmail}" style="color:#c9a84c;">${guestEmail}</a></p>`
    : '';
  ...
  const inner = `
    ...
    <h1 style="font-family:Georgia,serif;font-size:1.8rem;font-weight:400;color:#2a2218;margin:0 0 16px;line-height:1.3;">${guestName} sent you a message</h1>
    <p style="font-size:0.85rem;color:#6b5c45;line-height:1.6;margin:0 0 12px;">Regarding ${party.child_name}'s ${ageStr}:</p>
    <p style="font-size:0.9rem;color:#2a2218;background:#fff3e0;border-left:3px solid #c9a84c;padding:14px 16px;border-radius:0 6px 6px 0;margin:0 0 16px;font-style:italic;">"${message}"</p>
    ${replyRow}
