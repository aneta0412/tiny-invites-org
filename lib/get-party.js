...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (data.party_date) {
      // Compare in UTC: party_date is a date-only string; parsing it appends
      // UTC midnight, so "today" must also be UTC midnight.
      const today = new Date();
      today.setUTCHours(0, 0, 0, 0);
      if (new Date(`${data.party_date}T00:00:00Z`) < today) {
