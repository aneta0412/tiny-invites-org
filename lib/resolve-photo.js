...
export default async (req, context) => {
  ...
  // Parse and check the HOSTNAME — a substring check like url.includes(domain)
  // is bypassable (e.g. https://evil.com/?x=photos.google.com) and turns this
  ...
  const hostOk  = ['http:', 'https:'].includes(parsed.protocol)
  ...
  try {
    const res = await fetch(url, {
      ...
      redirect: 'follow',
      signal: AbortSignal.timeout(8000),   // don't hang the function on a slow fetch
    });
