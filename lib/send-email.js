...
export function welcomeEmailHtml({ child_name, age, venue, party_date, dashboard_token, party_id }) {
  ...
  return base(`
    ...
    <h1 style="font-family:Georgia,serif;font-size:1.9rem;font-weight:400;color:#2a2218;margin:0 0 14px;line-height:1.25;">${esc(child_name)}'s ${ageStr}<br>is all set! 🎉</h1>
    <p style="font-size:0.88rem;color:#6b5c45;line-height:1.75;margin:0 0 20px;">
      Your RSVP page is live and your QR code is ready to share. Invite your guests and then relax —
      we'll handle the rest from here.
    </p>
    ${dateBlock}
    ${venue ? `<div style="background:#f5edda;border-radius:10px;padding:13px 18px;margin-bottom:20px;font-size:0.84rem;color:#6b5c45;">📍 <strong style="color:#2a2218;">${esc(venue)}</strong></div>` : ''}

    <!-- Feature highlights -->
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
      <tr>
        <td style="padding:0 0 10px;">
          <div style="background:#f5edda;border-radius:10px;padding:14px 16px;display:flex;align-items:flex-start;gap:12px;">
            <span style="font-size:1.1rem;flex-shrink:0;line-height:1.3;">✉️</span>
            <div>
              <p style="font-size:0.82rem;font-weight:500;color:#2a2218;margin:0 0 3px;">RSVPs straight to your inbox</p>
              <p style="font-size:0.78rem;color:#6b5c45;margin:0;line-height:1.55;">You'll get an email the first time a guest responds each day, then a tidy summary if there are more. No response — no email.</p>
            </div>
          </div>
        </td>
      </tr>
      <tr>
        <td style="padding:0 0 10px;">
          <div style="background:#f5edda;border-radius:10px;padding:14px 16px;display:flex;align-items:flex-start;gap:12px;">
            <span style="font-size:1.1rem;flex-shrink:0;line-height:1.3;">⏰</span>
            <div>
              <p style="font-size:0.82rem;font-weight:500;color:#2a2218;margin:0 0 3px;">Guests are reminded automatically</p>
              <p style="font-size:0.78rem;color:#6b5c45;margin:0;line-height:1.55;">Every confirmed guest gets a reminder email 3 days before the party. You can add your own note to it from your dashboard.</p>
            </div>
          </div>
        </td>
      </tr>
      <tr>
        <td style="padding:0;">
          <div style="background:#f5edda;border-radius:10px;padding:14px 16px;display:flex;align-items:flex-start;gap:12px;">
            <span style="font-size:1.1rem;flex-shrink:0;line-height:1.3;">📢</span>
            <div>
              <p style="font-size:0.82rem;font-weight:500;color:#2a2218;margin:0 0 3px;">Message everyone in one click</p>
              <p style="font-size:0.78rem;color:#6b5c45;margin:0;line-height:1.55;">Venue changed? Need to share parking info? Use <em>Send update now</em> in your dashboard and every confirmed guest gets your message instantly.</p>
            </div>
          </div>
        </td>
      </tr>
  ...
  `, { deleteToken: dashboard_token });
}


// ── 2. Guest confirmation email (NO host footer — recipient is a guest) ──────
export function guestConfirmationHtml({ party, response }) {
  ...
  if (!attending) {
    return base(`
      ...
      <h1 style="font-family:Georgia,serif;font-size:1.9rem;font-weight:400;color:#2a2218;margin:0 0 14px;line-height:1.25;">Thanks for letting us know, ${esc(response.guest_name)}</h1>
      <p style="font-size:0.88rem;color:#6b5c45;line-height:1.75;margin:0;">
        ...
        <strong>${esc(party.child_name)}'s ${ageStr}</strong>. We'll be thinking of you and hope to celebrate together another time. 💛
      </p>
    `);
  }
  ...
  return base(`
    ...
    <h1 style="font-family:Georgia,serif;font-size:1.9rem;font-weight:400;color:#2a2218;margin:0 0 14px;line-height:1.25;">See you there, <em style="color:#c9a84c;">${esc(response.guest_name)}!</em></h1>
    <p style="font-size:0.88rem;color:#6b5c45;line-height:1.75;margin:0 0 18px;">
      Your RSVP for <strong>${esc(party.child_name)}'s ${ageStr}</strong> is confirmed.
      ${party.venue ? `We'll see you at <strong>${esc(party.venue)}</strong>.` : ''}
    ...
    ${response.allergies ? `<div style="background:#fff3e0;border-left:3px solid #c9a84c;border-radius:0 8px 8px 0;padding:12px 16px;font-size:0.82rem;color:#6b5c45;">⚠️ Dietary note recorded: <strong>${esc(response.allergies)}</strong></div>` : ''}
  `);
}
...
export function rsvpNotificationHtml({ party, response }) {
  ...
  return base(`
    ...
    <h1 style="font-family:Georgia,serif;font-size:1.9rem;font-weight:400;color:#2a2218;margin:0 0 18px;line-height:1.25;">${esc(response.guest_name)}<br><em style="color:#c9a84c;">${status}</em></h1>
    <div style="background:#f5edda;border-radius:10px;padding:16px 18px;margin-bottom:20px;">
      ...
      ${response.allergies ? `<p style="font-size:0.82rem;color:#6b5c45;margin:0;">⚠️ <strong>Dietary / note:</strong> ${esc(response.allergies)}</p>` : `<p style="font-size:0.82rem;color:#a89880;margin:0;">No dietary requirements noted.</p>`}
    </div>
...
export function digestEmailHtml({ party, responses }, opts = {}) {
  ...
  const rows = responses.map(r => {
    ...
    const emailCell = includeEmails
      ? `<td style="padding:9px 12px;font-size:0.74rem;color:#6b5c45;border-bottom:1px solid #f5edda;">${r.guest_email ? `<a href="mailto:${esc(r.guest_email)}" style="color:#c9a84c;text-decoration:none;">${esc(r.guest_email)}</a>` : '—'}</td>`
      : '';
    return `<tr>
