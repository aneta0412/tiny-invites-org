...
// Add this missing helper function right here:
...
export function welcomeEmailHtml({ child_name, age, venue, party_date, dashboard_token, party_id }) {
  ...
  return base(`
    ...
    ${dateBlock}
    ${venue ? `<div style="background:#f5edda;border-radius:10px;padding:13px 18px;margin-bottom:20px;font-size:0.84rem;color:#6b5c45;">📍 <strong style="color:#2a2218;">${venue}</strong></div>` : ''}
...
export function digestEmailHtml({ party, responses }, opts = {}) {
  ...
  const rows = responses.map(r => {
    ...
    const emailCell = includeEmails
      ? `<td style="padding:9px 12px;font-size:0.74rem;color:#6b5c45;border-bottom:1px solid #f5edda;">${r.guest_email ? `<a href="mailto:${r.guest_email}" style="color:#c9a84c;text-decoration:none;">${r.guest_email}</a>` : '—'}</td>`
      : '';
    return `<tr>
  ...
  return base(`
    ...
    <h1 style="font-family:Georgia,serif;font-size:1.9rem;font-weight:400;color:#2a2218;margin:0 0 10px;line-height:1.25;">${total} response${total!==1?'s':''} so far<br>for <em style="color:#c9a84c;">${party.child_name}'s party</em></h1>
    ${partyDateLine}
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:8px;">
      <tr>
