...
// Pretty-print a 24-hour "HH:MM" start time for emails: "14:00" -> "2pm".
// Returns '' for null/undefined/unparseable input.
export function formatPartyTime(val) {
  if (!val) return '';
...
export function formatTimeRange(start, durMin) {
  if (!start) return '';
  const s = formatPartyTime(start);
...
export function digestEmailHtml({ party, responses }, opts = {}) {
  ...
  const partyTimeStr = formatTimeRange(party.party_time, party.party_duration_min);
  const partyDateLine = party.party_date
    ? `<p style="font-size:0.85rem;color:#6b5c45;margin:0 0 18px;">🗓️ <strong style="color:#2a2218;">${formatPartyDate(party.party_date)}${partyTimeStr ? ` · ${partyTimeStr}` : ''}</strong></p>`
    : '';
