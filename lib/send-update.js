...
<table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
  ...
  <tr><td style="background:#fff9f2;border:1px solid #f5edda;border-radius:14px;overflow:hidden;">
    <table width="100%" cellpadding="0" cellspacing="0">
      ...
      <tr><td style="padding:32px 40px 36px;">
        ${hostCopy ? `<table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 18px;"><tr><td style="background:#eef4ef;border:1px solid #cfe0d4;border-radius:12px;padding:14px 16px;font-size:13px;color:#3f6b50;line-height:1.6;">📋 <strong>Your host copy.</strong> This is the update your confirmed guests just received.</td></tr></table>` : ''}
        <p style="font-size:10px;letter-spacing:0.18em;text-transform:uppercase;color:#c9a84c;margin:0 0 12px;">A message from the host 📢</p>
        <h1 style="font-family:Georgia,serif;font-size:29px;font-weight:400;color:#2a2218;margin:0 0 16px;line-height:1.3;">Update about ${escapeHtml(childName)}'s party</h1>
        <p style="font-size:14px;color:#6b5c45;margin:0 0 6px;">Hi <strong>${escapeHtml(guestName)}</strong>,</p>
        <p style="font-size:14px;color:#2a2218;line-height:1.8;margin:0 0 16px;white-space:pre-line;">${escapeHtml(message)}</p>
        ${note ? `<table width="100%" cellpadding="0" cellspacing="0" style="margin-top:4px;"><tr><td style="background:#fff3e0;border-left:3px solid #c9a84c;padding:14px 16px;border-radius:0 6px 6px 0;">
          <p style="font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:#8a6d00;margin:0 0 6px;">Note from the host</p>
          <p style="font-size:14px;color:#2a2218;line-height:1.7;margin:0;white-space:pre-line;">${escapeHtml(note)}</p>
        </td></tr></table>` : ''}
  ...
  <tr><td style="text-align:center;padding:20px 0 0;">
    ...
    <p style="font-size:10px;color:#c8bba8;margin:6px 0 0;">
      ${hostCopy
...
export default async function sendUpdate(req, res) {
 ...
 // Fetch attending guests with emails — excluding unsubscribed guests
  const { data: guests, error: guestsError } = await supabase
    ...
    .neq('guest_email', '')
    .or('reminder_optin.is.null,reminder_optin.eq.true'); // NULL = never opted out
  if (guestsError) {
  ...
  // ── Host copy ───────────────────────────────────────────
  // Send the host a copy of the update they just sent, so they have a record
  // of what guests received. Best-effort — never affects the guest sends.
