...
export default async function handler(req, res) {
  ...
  try {
    ...
    const guest_name = sanitiseString(body.guest_name, 100) || 'Guest';
    ...
    const reminder_optin        = body.reminder_optin === true || body.reminder_optin === 'true';
    ...
    // ── 2. Duplicate check ────────────────────────────────────
    let _rsvpChangedFrom = null; // set if guest flips yes→no or no→yes
    ...
    if (existing) {
      // ── Resend-only path: guest fixing a typo / not receiving confirmation ──
      // Update their email on the existing record and resend the confirmation.
      // Do NOT notify the host — they already know this guest is coming.
      if (resend_only && guest_email) {
        await supabase
          .from('guest_responses')
          .update({ guest_email, reminder_optin: true })
          .eq('id', existing.id);
        ...
        if (fullResponse) {
          sendEmail({
            to:      guest_email,
            subject: `You're confirmed for ${party.child_name}'s party! 🎉`,
            html:    guestConfirmationHtml({ party, response: { ...fullResponse, attending: 'yes' } }),
          }).catch(e => console.error('Resend confirmation failed:', e.message));
        }

        return res.status(200).json({ success: true, resent: true, id: existing.id });
      }

      // ── Attending status changed (yes→no or no→yes) ──────────
      // Delete the old record and fall through to a fresh INSERT so that:
      //   • a new created_at is written (dashboard "today" count updates correctly)
      //   • the host notification fires through the normal path below
    ...
    if (sendIndividual) {
      ...
      emailPromises.push(
        sendEmail({
          ...
          html: rsvpNotificationHtml({ party, response, todayCount, changedFrom: _rsvpChangedFrom }),
