...
export default async function handler(req, res) {
  ...
  try {
    ...
    // hold_host_email=true means this is a decline where the guest may add
    // a note. The host email is deferred to send-decline-note.js which fires
    // either when the guest sends a note or after a 30s timeout.
    const holdHostEmail = body.hold_host_email === true || body.hold_host_email === 'true';

    // ── 5. Send emails ────────────────────────────────────────
    const emailPromises = [];

    if (sendIndividual && !holdHostEmail) {
      emailPromises.push(
        sendEmail({
