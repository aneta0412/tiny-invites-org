...
export default async function handler(req, res) {
  ...
  try {
    const body = req.body || {};
    const { token, guest_id, action, attending, guest_count,
            guest_count_children, guest_count_adults, guest_count_babies,
            allergies } = body;
    ...
    if (guestError || !guest) return res.status(404).json({ error: 'Guest not found' });
    if (guest.party_id !== party.party_id) return res.status(403).json({ error: 'Guest does not belong to this party' });
    ...
    if (action === 'edit') {
      ...
      if (guest_count !== undefined) {
        ...
        // Keep in step with submit-rsvp.js (max 30 per response).
        if (!Number.isInteger(n) || n < 0 || n > 30)
          return res.status(400).json({ error: 'Invalid guest_count' });
        updates.guest_count = n;
      }

      if (guest_count_children !== undefined) {
        if (!isValidCount(guest_count_children))
          return res.status(400).json({ error: 'Invalid guest_count_children' });
        updates.guest_count_children = Number(guest_count_children);
      }

      if (guest_count_adults !== undefined) {
        if (!isValidCount(guest_count_adults))
          return res.status(400).json({ error: 'Invalid guest_count_adults' });
        updates.guest_count_adults = guest_count_adults !== null ? Number(guest_count_adults) : null;
      }

      if (guest_count_babies !== undefined) {
        if (!isValidCount(guest_count_babies))
          return res.status(400).json({ error: 'Invalid guest_count_babies' });
        updates.guest_count_babies = guest_count_babies !== null ? Number(guest_count_babies) : null;
      }
